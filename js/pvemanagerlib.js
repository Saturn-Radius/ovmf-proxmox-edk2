...
Ext.define('PVE.Utils', { utilities: {
    ...
    noSubKeyHtml: 'Welcome. Server interface is logged and monitored so please play nice.',
...
Ext.define('PVE.window.LoginWindow', {
	...
	control: {
		...
		specialkey: function(f, e) {
	      ...
	      change: function(f, value) { //set realm if root or not
        	 var rf = this.lookupReference('roleField');
	         if(value === "root"){
        	    rf.setValue("pam");
	         }
        	 else {
    ...
    title: gettext('Canaca-Com VE Login'),
...
Ext.define('PVE.tree.ResourceTree', {
	...
	var defaults;
	if (info.groupbyid){
	   if (Proxmox.UserName !== 'root@pam')//remove the groupbyid on left pannel
		return null;
...
Ext.define('PVE.qemu.Config', {
	...
	var migrateBtn = Ext.create('Ext.Button', {
	    ...
	    hidden: !caps.vms['VM.Migrate'], //(PVE.data.ResourceStore.getNodes().length < 2),
	    handler: function() {
		var win = Ext.create('PVE.window.Migrate', {
		    vmtype: 'qemu',
		    nodename: nodename,
...
Ext.define('PVE.dc.Support', {
    ...
    invalidHtml: 'Welcome' + PVE.Utils.noSubKeyHtml,
...
Ext.define('PVE.Workspace', {
    ...
    title: 'Canaca-COM Virtual Environment',
...
Ext.define('PVE.StdWorkspace', {
	...
	var searchField = Ext.createWidget('pveGlobalSearchField', {tree: rtree});
	...
	sprovider.on('statechange', function(sp, key, value) {
		...
		createVM.setHidden(!caps.vms['VM.Allocate']);
                createCT.setHidden(!caps.vms['VM.Allocate']);
                searchField.setHidden(!caps.vms['VM.Allocate']);
                selview.setHidden(!caps.vms['VM.Allocate']);
	    }
	});
